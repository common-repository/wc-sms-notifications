...
function enableValidateBtn(obj,otp_length=0)
{
	if(jQuery(obj).val().match(/^\d{4,8}$/)) {
			jQuery(".wcsmsn_otp_validate_submit,obj").removeAttr("style");
			jQuery(".wcsmsn_otp_validate_submit,obj").removeAttr("disabled");
			
	}
	else
	{
		jQuery(".wcsmsn_otp_validate_submit,obj").css({"color":"grey","pointer-events":"none"});
	}
}
...
function wcsmsn_otp_timer(obj,otp_timer=15)
{
	...
	obj.find(".wcsmsn_timer").show();
	obj.find(".wcsmsn_timer").html(timer(otp_timer)+" sec");
	obj.find(".wcsmsn_resend_btn").text("Resend");
	var counter = otp_timer;
	 interval = setInterval(function() {
		counter--;
		 var places = (counter < 10 ? "0" : "");
		obj.find(".wcsmsn_timer").html(timer(counter)+" sec");
		if (counter == 0) 
		{
			counterRunning=false;
			obj.find(".wcsmsn_timer").hide();
			var cssString = "pointer-events: auto; cursor: pointer; opacity: 1; float:right"; 
			obj.find(".wcsmsn_resend_btn").attr("style",cssString);
...
function saInitOTPProcess(obj,action_url, data_obj,otp_resend_timer=15,success_cb=null,failure_cb=null)
{
			 ...
			 var currentModel 	 = jQuery(obj).parents("form").find(".wcsmsnModal"); 
			   ...
			   $mo.ajax({
					...
					currentModel.show(), 
					currentModel.find(".wcsmsn_validate_field").show(),
					wcsmsn_otp_timer(currentModel,otp_resend_timer),
					counterRunning=true,
					((typeof success_cb == "function") ? success_cb(o) : "" )
					):
					(
					currentModel.find(".wcsmsn_validate_field").hide(),
...
function saInitBlockOTPProcess(obj,action_url, data_obj,otp_resend_timer=15,success_cb=null,failure_cb=null)
{
	...
	var prev_btn_text = cur_btn.val();
	cur_btn.val("Please wait...");
	var currentModel 	 = jQuery(obj).parents(".entry-content").find(".wcsmsnModal"); 
	//var currentModel 	 = jQuery(obj).parents(".entry-content").find('.wcsmsnModal'); 
	if(counterRunning){currentModel.show();return false;}
...
function wcsmsn_validateOTP(obj,action_url,data_obj,callback)
{
	$mo  =jQuery;
	...
	var current_form = $mo(obj).parents("form");
	$mo.ajax({
		//url:"'.site_url().'/?option=wcsmsn-validate-otp-form",
		url:action_url,
		type:"POST",
		...
		(
			current_form.find('.wcsmsnModal').hide(),
...
function wcsmsn_validateBlockOTP(obj,action_url,data_obj,callback)
{
	$mo  =jQuery;
	var current_btn = $mo(obj);
	current_btn.attr("disabled",true);
	var current_form = $mo(obj).parents(".wcsmsnModal");
	$mo.ajax({
		//url:"'.site_url().'/?option=wcsmsn-validate-otp-form",
		url:action_url,
		type:"POST",
...
jQuery(document).on("change",".wcsmsnModal .otp-number",function(e) {
	var otp_length = 4;
		var txtData = [];
		var parent = jQuery(this).parents(".wcsmsnModal");
